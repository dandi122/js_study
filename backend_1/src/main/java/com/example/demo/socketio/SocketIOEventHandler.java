 ...
 *  curl "https://openapi.naver.com/v1/search/news.xml?query=%EC%A3%BC%EC%8B%9D&display=10&start=1&sort=sim" \
    -H "X-Naver-Client-Id: 1QSxRtIDsO2dOZ20Q66C" \
    -H "X-Naver-Client-Secret: uuSOJs8yaG" -v
 *  
 *  
 */
...
public class SocketIOEventHandler {
	...
	public SocketIOEventHandler(SocketIOServer server) {
		...
		server.addEventListener("cvr_news", String.class, new DataListener<String>() {
			...
			public void onData(SocketIOClient client, String data, AckRequest ackSender) throws Exception {
				...
				// 실습 => 검색결과를 클라이언트에게 전송 이벤트명 "svr_news
	...
	private String naverOpenApiForNews( String keyword ) {
        ...
        String apiURL 		= "https://openapi.naver.com/v1/search/news.json?display=10&start=1&sort=sim&query=" + keyword;
