...
- ts
    - 기본타입
        ...
        - ... 잘 사용되지 않지만 존재하는 타입 있음
    - 복합타입 => 배열, 클래스형 => 클래스형(가장 본질적인 타입)
        - array(ts) vs Array(js) : 배열 타입 -> 배열의 멤버는 모두 동일 타입임(수치형배열, 문자열배열...)
        - tuple : 고정된 요소수를 가진 배열, 튜플 타입, ts만 추가, 복합 타입으로 멤버구성 가능
    ...
    - 고급타입 : 사용자 정의 타입 
        ...
        - intersection : 여러 타입을 결합하여, 하나의 타입을 만든다
            ...
            ```
        - type 별칭 : 사용자 정의 타입 <-> interface와 대비해서 자주 사용된다(별다섯개)
...
// 3. ts
// 변수명:타입주석(Type Annotation) => 제약을 준다 => 오류 발생 범위를 줄인다
let t1:number  = 1;
