...
modifier(엑세스(pri|pro|def|pub)|일반(fin|sta|abs|..)) 리턴타입 메소드명(매개변수) {
    [수행문]
    [return [반환값]]
}
 */
// 타입 주석은 무조건 :타입주석 방식으로 뒤쪽으로 표현
...
// 20241007
// 함수 == Function 클레스의 인스턴스 => 함수는 객체다
let sub = new Function('a', 'b', 'return a - b');  // 이렇게 표현은 잘 하지 않음
...
// 계산 
// 조급한 계산 : eager evaluation  => 1+2 => 3 컴파일러가 바로 처리(해석 가능함)
// 느긋한 계산 : lazy evaluation => function cal(a, b){return a+b} 
//                              => a, b가 명확하지 않다, 연산 바로 하지 않는다
//                              => 우리가 구현하는 함수들은 대부분 느긋한 계산에 속한다

// 익명함수 응용 표현 + 조급한 계산 적용
let sub3 = (function(a10, b10){
    return a10 + b10;
})(1, 2);
